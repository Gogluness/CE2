...
<?php
	...
	try
	{
	    ...
	    $preparedStatement = "UPDATE `Produit` SET `CUP` = ?, `Description` = ?, `IDModele` = ?, `Nom` = ?, `NomCompagnie` = ?, `PrixCout` = ?, `PrixVente` = ?, `Quantite` = ?, `ImgPath` = ? WHERE ID = ?";
	    $stmt = $conn->prepare($preparedStatement);
	    $stmt->execute(array($CUP,$Description,$Modele,$Nom,$Compagnie,$PrixCout,$PrixVente,$Qte,$ImgPath,$formID));
	    $conn=null;
	}
